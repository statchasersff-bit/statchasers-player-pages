/**
 * InjuryTab — the player-profile Injury tab. Composes four cards:
 *   1. Injury Risk Profile  — durability grade box + plain-language risk + stats
 *   2. Availability Timeline — season-by-season week grid (the centerpiece)
 *   3. Body Map + Risk Breakdown — where/why, side by side
 *   4. Full Injury Log       — filterable, collapsible NFL + college list
 *
 * Props:
 *   injury       — PlayerInjuryHistory | null (from fetchPlayerInjuryHistory)
 *   loading/error
 *   playerName
 *   knownSeasons — seasons the player is known to have played (so injury-free
 *                  years render as "Full season"); pass [] if you don't have it.
 *   weeklyPlayed — real weeks played per season, for authoritative availability;
 *                  optional (the timeline falls back to an estimate without it).
 */
...
const INJURY_BODYPART_CHIP = "bg-slate-500/10 text-slate-600 dark-any:text-slate-300";
const INJURY_MISSED_CHIP = "bg-rose-500/10 text-rose-600 dark-any:text-rose-400";
...
// Original phrasing for a college / pre-NFL entry from its structured fields.
...
// The severity shown on a log row: missed-time injuries use the display tier
// (incl. "season-ending"); played-through injuries fall back to clinical source.
  ...
  return (
    <span className="ml-auto flex flex-wrap items-center justify-end gap-1">
      ...
      {missed > 0
        ...
        : <span className={cn(INJURY_BADGE, "bg-emerald-500/10 text-emerald-600 dark-any:text-emerald-400")}>Played through</span>}
    </span>
  );
}
...
// Full injury log — collapsed by default; detail rows live behind a toggle so
// the page leads with the visuals.
...
function FullInjuryLogCard({ injury }: { injury: PlayerInjuryHistory }) {
  ...
  // Group by season (newest first); within a season, latest week first.
