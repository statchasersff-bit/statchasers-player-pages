/**
 * injuryDerivations — pure functions that turn a PlayerInjuryHistory into the
 * summary stats, risk breakdown, and narratives the Injury tab renders. No
 * React, no I/O — unit-test these directly. All heuristics are simple,
 * transparent thresholds (explainable, not a black box).
 */
...
// Body-part regions for summaries / the risk table. Each injury is matched by
// keyword against its body part (falling back to type, so e.g. a "Concussion"
// with no body part still lands in Head/Neck).
...
export const RISK_LEVEL_STYLE: Record<RiskLevel, { label: string; text: string; bar: string; score: number }> = {
  low:      { label: "Low",      text: "text-emerald-600 dark-any:text-emerald-400", bar: "bg-emerald-500", score: 25 },
  moderate: { label: "Moderate", text: "text-amber-600 dark-any:text-amber-400",     bar: "bg-amber-500",   score: 60 },
  high:     { label: "High",     text: "text-rose-600 dark-any:text-rose-400",        bar: "bg-rose-500",    score: 90 },
};
...
export function summarizeInjuries(injury: PlayerInjuryHistory): InjurySummary {
  ...
  // Primary risk driver — the single injury responsible for the most missed
  // time, breaking ties by severity.
...
// One-line interpretation under the Availability Timeline: biggest sources of
// missed time, plus regions that were played through.
...
// Heuristic risk breakdown — each row maps a transparent threshold to a level.
export function computeRiskBreakdown(injury: PlayerInjuryHistory): { label: string; level: RiskLevel }[] {
  ...
  // Recent injury risk — injuries in the two most recent seasons present.
  ...
  // Recurring injury risk — most repeated body part.
  ...
  // Games missed impact — career games lost to injury.
  ...
  // Body-part concentration — share of injuries in the single most-affected area.
...
// Maps a 0–100 durability score to a plain-language risk word. Lower = riskier.
export function durabilityRiskLabel(score: number): { word: string; className: string } {
  if (score >= 80) return { word: "Low",      className: "text-emerald-600 dark-any:text-emerald-400" };
  if (score >= 60) return { word: "Moderate", className: "text-amber-600 dark-any:text-amber-400" };
  if (score >= 40) return { word: "Elevated", className: "text-orange-600 dark-any:text-orange-400" };
  return { word: "High", className: "text-rose-600 dark-any:text-rose-400" };
}
