/**
 * InjuryBodyMap — a stylized body silhouette plotting where a player's career
 * injuries occurred. Injuries are grouped by body region; each region shows one
 * marker, colored by its worst severity, with a count when multiple land there
 * and a glow ring when any cost games. Reuses the Availability Timeline's
 * normalization + severity model so both sections stay consistent.
 */
...
// Central regions sit on the midline (one coord); lateral regions resolve to a
// left/right coordinate when the side is known, or a centered generic coord
// (x = 50) when it isn't.
const CENTRAL_REGIONS = new Set<BodyRegion>(["head", "neck", "chest", "ribs", "back", "hip"]);

// Coordinate space is 0–100 on both axes (the card is square), so a marker's
// left/top percentage lines up with the silhouette's SVG viewBox.
...
/** Match an injury to a body region by keyword (body part first, then text). */
...
/** Resolve a left/right side from the explicit field or the injury text. */
...
/** The BODY_REGION_COORDS key an injury maps to (sided for lateral regions). */
...
/** Lookup coordinates for a resolved region key. */
...
/** The worst severity across a set of injuries. */
...
// "played" markers share the minor (teal) color.
...
/**
 * Per-region rollup (left/right combined) — injuries, games missed, worst
 * severity, and a simple risk level — sorted by games missed then count.
 */
...
const REGION_RISK_STYLE: Record<RegionRisk, { label: string; text: string }> = {
  low:      { label: "Low",      text: "text-emerald-600 dark-any:text-emerald-400" },
  moderate: { label: "Moderate", text: "text-amber-600 dark-any:text-amber-400" },
  high:     { label: "High",     text: "text-rose-600 dark-any:text-rose-400" },
};

/** Group injuries by resolved region key; events with no region are dropped. */
...
// Glow ring color per severity, applied when a region caused missed games.
...
/** Class string for a marker circle. */
...
/** Plain-language summary of where the injuries cluster. */
export function buildBodyMapSummary(events: InjuryEvent[]): string {
  ...
  let upper = 0;
  let lower = 0;
  let missedRegion: BodyRegion | null = null;
  let missedMax = 0;
...
// Subtle silhouette — faint capsule limbs with a softly-filled head and tapered
// torso. Deliberately low-contrast so the injury markers stay the focal point.
...
// Plain-text fallback (screen readers / aria-label).
...
// One styled row per injury inside the hover tooltip.
...
function RegionMarker({
  regionKey, events, selected, onToggle,
}: {
  regionKey: string;
  events: InjuryEvent[];
  selected: boolean;
          ...
          return (
            <div key={i} className="text-[12px] leading-snug">
              ...
              <span className="text-muted-foreground">
                ...
                {" "}· {missed > 0 ? <span className="font-semibold text-rose-600 dark-any:text-rose-400">{missed} games missed</span> : "played through"}
              </span>
...
export function InjuryBodyMap({
  ...
  return (
    <Card className={cn("p-5", className)}>
      ...
      {/* Inline region detail (tap target on mobile, click on desktop) */}
      {selectedEvents && <RegionInjuryList events={selectedEvents} onClose={() => setSelectedKey(null)} />}

      {/* Most affected regions — injuries, games missed, and risk per area */}
      {topRegions.length > 0 && (
        <div className="mt-4">
