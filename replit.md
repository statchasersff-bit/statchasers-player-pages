...
- **Data Pipeline:** A series of Node.js scripts (`buildPlayersIndex.js`, `buildIndexedPlayers.js`, `buildGameLogs.js`, `buildGameScores.js`, `buildDynastyRankings.js`, `buildStaticApi.js`) are used to fetch, normalize, and store data as static JSON files. This pre-generates API responses, eliminating the need for a live API server for most data, enhancing performance and scalability.
- **Indexed Player System:** Focuses on 352 fantasy-relevant players (11 per team) for core content, with a comprehensive directory for all players. Ranking within this system is based on depth chart order, active status, experience, and name.
