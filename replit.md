...
## Recent Changes
- 2026-02-21: Sustainability score dispersion overhaul: z-score standardized efficiency signal (FP/Usage divided by posStdDev, multiplier ×12), convex TD penalty scaling (1.8× acceleration >1 std dev), re-anchored baseline 50→57, QB rushing floor signal (min(10, (rush_att/g - 4) × 2)), reduced fpPerUsage stdDev floor 0.5→0.01; QB range expanded from 36–67 to 39–81 (elite=75+, strong=65-74, neutral=50-64, fragile=<50)
- 2026-02-21: QB calibration: Y/A benchmark fixed (pass_yds/pass_att not pass_yds/pass_cmp); stability uses blended pass_att + 2×rush_att; momentum uses cmp_pct(×1) replacing team_pass_att(×2); sustainability efficiency uses FP/(pass_att + 2×rush_att) with league cohort benchmarks; momentum delta capped ±25 per metric
